...
GWAS_ori <- read.table("/Users/xiyas/ATPM_Project/database-v3/gwas_catalog_v1.0.2-associations_e112_r2024-07-27.tsv",header = TRUE,fill = TRUE,sep = "\t",quote = "")
...
#remove p value >5e-8 and also OR with NA valuenew start from 2024.8 =========
GWAS <- GWAS %>% filter(P.VALUE<5e-8)
GWAS <- GWAS %>% filter(!(is.na(GWAS$OR.or.BETA)))
GWAS <- GWAS %>% filter(OR.or.BETA!=0)

#hist(GWAS$OR.or.BETA)
...
Merged_GWAS_vcf <- read.delim("/Users/xiyas/V2_Genome_reporting/database-file-v2/Merged_GWAS_vcf.txt")
