...
merge_table_swe <- fread(file= '/Users/xiyas/pgsc_calc/vcf-swedish-with-merge/merge_table_swe.txt',quote = FALSE,sep = '\t')
merge_table_tur <- fread(file= '/Users/xiyas/V2_Genome_reporting/R_scripts/merge_table.txt',quote = FALSE,sep = '\t')
...
Tier_1_VIP$key <- paste(Tier_1_VIP$`Variant/Haplotypes`, Tier_1_VIP$`Genotype/Allele`, sep = "_")
...
kGAF <- fread('/Users/xiyas/pgsc_calc/PGx-VIP.txt',sep = '\t',header = TRUE)
...
# several rare PGx variants were only identified in TR, not in SW 1000G and Gnomad v3(Supplementary Table 2). =========
