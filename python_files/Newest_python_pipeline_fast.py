...
if path == 'local':
    ...
    if run_GWAS_Pharmaco == True:
        ...
        print("GWAS,Pharmaco matching will run")
    ...
    diseaesDBfile = '/proj/snic2020-16-69/nobackup/WGS_SZA/database-file-v2/DiseaseDB_version_2_Beta_curated.txt'
    ...
    diseaesDBfile = '/mnt/SZAPORTAL/database-file-v2/DiseaseDB_version_2_Beta_curated.txt'
    ...
    #start processing
        ...
        for j in range(0,len(iText)):
            ...
            if jText[colNames_CSQ.index('Database_SZAID')] != '' and 'ClinP_LP_var' in jText[colNames_CSQ.index('Database_Type')].split('&'):
                #sys.exit('!')
                saveFlag1 = jText[colNames_CSQ.index('Database_SZAID')] != ''
                #MAX_AF to gnomad global AF, and new filters need to be applied here too
            elif jText[colNames_CSQ.index('Database_SZAID')] != '' and 'GWAS_var' in jText[colNames_CSQ.index('Database_Type')].split('&'):
                saveFlag3 = "GWAS_var"
            elif jText[colNames_CSQ.index('Database_SZAID')] != '' and 'Pharma_var' in jText[colNames_CSQ.index('Database_Type')].split('&'):
                saveFlag4 = "Pharma_var"
            elif (jText[colNames_CSQ.index('MAX_AF')] == '' or float(jText[colNames_CSQ.index('MAX_AF')])<0.05):
...
if run_GWAS_Pharmaco == True:
    from datetime import datetime
    now = datetime.now()
    newLineHeadings = ['Traits name', 'category', 'genes', 'variants', 'Description','Patient genotypes','Zygosity'];
    newLine = '\t'.join(newLineHeadings)+'\n'
    ...
    for i in range(0,len(reportE)):
        ...
        irs_id = iText[0].split("|")[colNames_CSQ.index('Existing_variation')]
        ...
        if "." not in reportE[i].split('\t')[9].split(':')[0]:
            ...
            newLine_temp ='\t'.join([iTraits,iCategory,iGenes,irs_id,iDescription,iGenotype,iZygo])+'\n'
