...
# GenomeReportingPipeline version 3.2 on SZA sercer
...
config = {
    "local": {
        ...
        "Pharma_dbfile_all":"/Users/xiyas/ATPM_Project/database-v3/Pharma_db_all.txt",
        "Haplotype_rs_ref":"/Users/xiyas/ATPM_Project/database-v3/PGx_annotation_all.levels.drugs.hap250220.txt",
    ...
    "sysmed": {
        ...
        "Pharma_dbfile": "/mnt/storage_pool/Genomics/Genome/database-files/Merged_Pharma_vcf_2024.txt",  # 更新路径
        "Pharma_dbfile_all": "/mnt/storage_pool/Genomics/Genome/database-files/Pharma_db_all.txt",  # 添加 Pharma_dbfile_all
        "Haplotype_rs_ref": "/mnt/storage_pool/Genomics/Genome/database-files/PGx_annotation_all.levels.drugs.hap250220.txt",  # 更新路径
...
haplotype_rs_set = set()
if run_Pharmaco:
    Pharma_db = read_db_file(cfg["Pharma_dbfile"])
    Pharma_db_all = read_db_file(cfg["Pharma_dbfile_all"])
    print("Pharmaco matching will run")
    haplotype_db = read_db_file(cfg["Haplotype_rs_ref"])
    haplotype_db = haplotype_db[haplotype_db['Genotype.Allele'].str.contains(r'\*', na=False)]
    # 构建单体型到rsID的映射
    haplotype_map = defaultdict(set)
    for _, row in haplotype_db.iterrows():
        haplo_id = row['Variant.Haplotypes']
        # 处理分号分隔的多值rsID
        rs_list = [r.strip().lower() for r in str(row['rsID']).split(';') if r.startswith('rs')]
        haplotype_map[haplo_id].update(rs_list)  # 使用update自动去重
    # 构建反向映射（rsID → 所属单体型）
    rs_to_haplotype = defaultdict(set)
    for haplo_id, rs_set in haplotype_map.items():
        for rs in rs_set:
            rs_to_haplotype[rs].add(haplo_id)
    # 统计rsID: 3090 variants in this db
    for rs in haplotype_db['rsID'].dropna():
        for r in str(rs).split(';'):
            clean_rs = r.strip().lower() 
            if clean_rs.startswith('rs'):
                haplotype_rs_set.add(clean_rs)
    # 构建annotation db (两个，一个是snp+genotype， 一个是gene+genotype （CYP2D6*1）)
    pharma_snp_dict = {}  # Key: (rsID, genotype)
    pharma_haplotype_dict = {}  # Key: (CYP2D6*2, etc)
    # 遍历 Pharma_db_all 的每一行
    for _, row in Pharma_db_all.iterrows():
        variant = row['Variant/Haplotypes']
        genotype = row['Genotype/Allele']
        gene = row['Gene']
        # 判断是单 SNP 还是单倍型
        if variant.startswith('rs'):
            # 单 SNP：键为 (rsID, genotype)
            key = (variant, genotype)
            # 如果键不存在，初始化一个空列表；否则追加当前行
            if key not in pharma_snp_dict:
                pharma_snp_dict[key] = []
            pharma_snp_dict[key].append(row)
        if '*' in variant:
        # 构建键：Gene + Genotype
            key = f"{gene}{genotype}"  # 例如 'CYP3A5*6'
            # 如果键不存在，初始化一个空列表；否则追加当前行
            if key not in pharma_haplotype_dict:
                pharma_haplotype_dict[key] = []
            pharma_haplotype_dict[key].append(row)
...
while tLine:
    ...
    for j in range(0,len(iText)):
        ...
        if run_Pharmaco:  
            ...
            # Haplotype
            rsID = jText[col_map['Existing_variation']].split('&')[0]
            if rsID in haplotype_rs_set:
                saveFlag4 = "Pharma_var" 
...
def _construct_output_line(match_row, iGenotype, iZygo, jrsID):
    """Helper function to construct the output line."""
    return '\t'.join([
        str(match_row['Drug(s)']),
        iGenotype,
        iZygo,
        str(match_row['Genotype/Allele']),
        "NA",  # SZAvarID placeholder
        str(jrsID),
        str(match_row['Annotation Text']),
        str(match_row['Score']),
        str(match_row['Phenotype Category']),
        str(match_row['Phenotype(s)']),
        str(match_row['Gene']),
        str(match_row['Level of Evidence']),
        str(match_row['Level Modifiers']),
        str(match_row['URL'])
    ]) + '\n'
...
if run_Pharmaco:
    ...
    with open(out_pharma, 'w') as f:
        f.write('\t'.join(newLineHeadings) + '\n')
    print(f'Processing {len(reportD)} pharmaco variants...')

    # 2. Collect all rsIDs in the sample
    sample_rsids = set()
    for line in reportD:
        fields = line.split('\t')
        info_part = fields[7]
        ...
        if csq_candidates:
            ...
            jText = iText[0].split('|')
            jrsID = jText[col_map['Existing_variation']].split('&')[0]
            sample_rsids.add(jrsID)

    # 3. Process each variant in reportD (SNP + Haplotype)
    processed_haplotypes = set()  # Track processed haplotypes to avoid duplicates
    for line in reportD:
        fields = line.split('\t')
        chrom, pos, id_, ref, alt = fields[:5]
        info_part = fields[7]
        # Extract rsID from INFO field
        csq_candidates = [s for s in info_part.split(';') if 'CSQ=' in s]
        if not csq_candidates:
            continue
        iText = csq_candidates[0].replace('CSQ=', '').split(',')
        jText = iText[0].split('|')
        jrsID = jText[col_map['Existing_variation']].split('&')[0]
        # Extract genotype and zygosity
        sample_geno_field = fields[9].split(':')[0]  # e.g., '0/1'
        if '.' in sample_geno_field:
            continue  # Skip missing genotypes
        # Build genotype string (e.g., 'A/G')
        try:
            iGenotypInd1 = int(sample_geno_field[0])
            iGenotypInd2 = int(sample_geno_field[2]) if len(sample_geno_field) > 2 else 0
            allele1 = ref if iGenotypInd1 == 0 else alt.split(',')[iGenotypInd1-1]
            allele2 = ref if iGenotypInd2 == 0 else alt.split(',')[iGenotypInd2-1]
            sorted_alleles = sorted([allele1, allele2])
            iGenotype = ''.join(sorted_alleles) 
            # Determine zygosity
            if allele1 == allele2:
                iZygo = 'Homozygous'
            elif iGenotypInd1 == 0 or iGenotypInd2 == 0:
            ...
            # Handle hemizygous case for males on chrX
            if check_male_flag and chrom == 'chrX' and iZygo == 'Heterozygous':
                iZygo = 'Hemizygous'
        except:
            continue  # Skip invalid genotypes
        # ===== 处理单 SNP 注释 =====
        key_snp = (jrsID, iGenotype)
        if key_snp in pharma_snp_dict:
            for match_row in pharma_snp_dict[key_snp]:
                newLine_temp = _construct_output_line(match_row, iGenotype, iZygo, jrsID)
                with open(out_pharma, 'a') as f:
                    f.write(newLine_temp)

        # ===== 处理单倍型（star allele）注释 =====
        if jrsID in rs_to_haplotype:
            for haplo_id in rs_to_haplotype[jrsID]:
                if haplo_id not in processed_haplotypes:
                    # 检查单倍型所有 rsID 是否存在于样本中
                    required_rsids = haplotype_map[haplo_id]
                    if required_rsids.issubset(sample_rsids):
                        print("we found a star allele",haplo_id)
                        if haplo_id in pharma_haplotype_dict:
                            print("start writing")
                            for match_row in pharma_haplotype_dict[haplo_id]:
                                # 使用单倍型的基因型（如 *1/*2）
                                haplo_genotype = match_row['Genotype/Allele']
                                newLine_temp = '\t'.join([
                                    str(match_row['Drug(s)']),
                                    haplo_genotype,  # e.g., *1/*2
                                    "NA",  # 根据实际单倍型逻辑调整
                                    haplo_genotype,
                                    "NA",
                                    haplo_id,  # 使用单倍型 ID 作为 rsID
                                    str(match_row['Annotation Text']),
                                    str(match_row['Score']),
                                    str(match_row['Phenotype Category']),
                                    str(match_row['Phenotype(s)']),
                                ...
                                with open(out_pharma, 'a') as f:
                                    f.write(newLine_temp)
                        processed_haplotypes.add(haplo_id)

    print(f"Generated pharmaco file: {out_pharma}")
...
# revel score fixed ----
for index, row in filtered_clingen_merge_df_ontology.iterrows():
    if pd.isna(row['REVEL']) or row['REVEL'] == '':
        revel_score_value = row['REVEL_score']
        if pd.notna(revel_score_value) and revel_score_value != '':
            parsed_value = extract_decimal_from_string(str(revel_score_value))
            if parsed_value is not None:
                #print(f"Updating row {index}: REVEL_score={revel_score_value} -> REVEL={parsed_value}")
                filtered_clingen_merge_df_ontology.at[index, 'REVEL'] = parsed_value

# Predicted Impact Reason column ----
# 初始化 Reason 列
filtered_clingen_merge_df_ontology['Reason'] = ''

# 遍历每一行，检查条件并填充 Reason 列
for index, row in filtered_clingen_merge_df_ontology.iterrows():
    reasons = []  # 用于记录满足的条件

    # 检查 MAX_AF 阈值
    if pd.isna(row['MAX_AF']) or float(row['MAX_AF']) < 0.05:
        # 检查 pathogenicity impact 条件
        if row['IMPACT'] == 'HIGH':
            reasons.append('HIGH_IMPACT')
        if pd.notna(row['ada_score']) and float(row['ada_score']) > 0.6:
            reasons.append('ADA_SCORE>0.6')
        if pd.notna(row['rf_score']) and float(row['rf_score']) > 0.6:
            reasons.append('RF_SCORE>0.6')
        if pd.notna(row['REVEL']) and float(row['REVEL']) > 0.75:
            reasons.append('REVEL>0.75')
        if pd.notna(row['SpliceAI_pred_DS_AL']) and float(row['SpliceAI_pred_DS_AL']) > 0.5:
            reasons.append('SpliceAI_AL>0.5')
        if pd.notna(row['SpliceAI_pred_DS_DG']) and float(row['SpliceAI_pred_DS_DG']) > 0.5:
            reasons.append('SpliceAI_DG>0.5')
        if pd.notna(row['SpliceAI_pred_DS_DL']) and float(row['SpliceAI_pred_DS_DL']) > 0.5:
            reasons.append('SpliceAI_DL>0.5')
        if pd.notna(row['SpliceAI_pred_DS_AG']) and float(row['SpliceAI_pred_DS_AG']) > 0.5:
            reasons.append('SpliceAI_AG>0.5')
        if pd.notna(row['BayesDel_addAF_score']) and float(row['BayesDel_addAF_score']) > 0.0692655:
            reasons.append('BayesDel_addAF>0.0692655')
        if pd.notna(row['BayesDel_noAF_score']) and float(row['BayesDel_noAF_score']) > -0.0570105:
            reasons.append('BayesDel_noAF>-0.0570105')
        if row['am_class'] == 'likely_pathogenic' and float(row['am_pathogenicity']) > 0.564:
            reasons.append('AM_PATHOGENICITY>0.564')
        if row['LoF'] == 'HC':
            reasons.append('LoF_HC')

    # 将满足的条件用分号连接，并填充到 Reason 列
    if reasons:
        filtered_clingen_merge_df_ontology.at[index, 'Reason'] = ';'.join(reasons)
